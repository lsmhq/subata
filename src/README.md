---
home: true
icon: home
title: Subata

heroImage:

bgImage: /6-light.svg
bgImageDark: /6-dark.svg
bgImageStyle:
  background-attachment: fixed

heroText: Subata
tagline: Wizard101 攻略 · 汉化 · 工具 · 社区

actions:
  - text: 开始探索
    link: ./main/index2.html
    type: primary

  - text: 新人入坑指南
    link: ./main/index4.html
    type: primary

  - text: 加入创作者计划
    link: ./template/start.md
    type: default

  - text: 社区交流（QQ）
    type: default
    link: https://qm.qq.com/cgi-bin/qm/qr?k=M3OaXkN0Na4dTCjhuRFOg4lYVFM-T7zf&jump_from=webapi&authKey=F5F8uIKs3dO114jF/vlVHYAzbVRUyEarkRZOF3xQq1xj6qL6eYm3RZmr+lhPx82E


highlights:

  # 第一屏：站点定位（替代原启动器宣传）
  - header: 欢迎来到 Subata
    description: 专注于 Wizard101 的中文攻略站，持续更新新手教程、深度玩法、汉化内容与实用工具。
    image: /images/subata-main.png
    bgImage: /subata/3-light.svg
    bgImageDark: /subata/3-dark.svg

    highlights:
      - title: 新人快速入坑
      - title: 全世界主线攻略
      - title: 团本与战斗机制
      - title: 汉化项目持续更新
      - title: 实用工具与资源整合


  # 第二屏：核心内容入口
  - header: 攻略导航
    description:
    bgImage: /subata/4-light.svg
    bgImageDark: /subata/4-dark.svg

    bgImageStyle:
      background-repeat: repeat
      background-size: initial

    features:

      - title: 入坑准备
        icon: star
        link: /main/index4.html

      - title: 新人基础指南
        icon: book
        link: /base/index1.html

      - title: 深入游戏指南
        icon: plane
        link: /deep/index1.html

      - title: 战斗玩法
        icon: fire
        link: /fight/index1.html

      - title: 特殊玩法（周常）
        icon: play
        link: /special/index1.html

      - title: 生产采集
        icon: hammer
        link: /manufacture/index1.html

      - title: 常用工具
        icon: tools
        link: /tools/index1.html

      - title: 日常休闲
        icon: sun
        link: /daily/index1.html

      - title: 旧攻略归档
        icon: archive
        link: /wordpress/index1.html

      - title: 其他内容
        icon: child
        link: /other/index1.html


  # 第三屏：轻量展示启动器，不抢主视觉
  - header: Subata 工具生态
    description: 除攻略内容外，我们也提供一些提升体验的小工具。
    bgImage: /subata/5-light.svg
    bgImageDark: /subata/5-dark.svg

    highlights:
      - title: 启动器下载
      - title: 自动皇冠答题
      - title: 账号状态查询
      - title: 个性化界面
      - title: 持续迭代更新


copyright: Subata
footer: Built with passion by Subata
---